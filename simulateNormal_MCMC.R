...
paramFile="./PE/PE_Hier6_Normal_0Exc";nChains=4;nExcluded=0;CIWidths=c(0.8,0.95);
outFile="./CI/CI_Hier6_Normal_0Exc.txt"
...
getCIs=function(paramFile,nChains,nExcluded,CIWidths=0.8,outFile){
  ...
  nuMatrix=matrix(Inf,nrow=nrow(mu),ncol=ncol(mu))#Don't read in nus. They're all Inf (i.e. normal)
  phi=t(paramMatrix[(n+1):(2*n),]);
  tau=t(paramMatrix[(2*n+1):(3*n),]);
  sigma=tau^(-1/2);
...
#A little-used function that just gets some sample trajectories for the country with the given index
getSomeTrajectories=function(paramFile,nExcluded,countryIndex,nTrajectories){
  #Sample call: getSomeTrajectories(paramFile="./PE/PE_Hier6_Normal_0Exc",nExcluded=0,countryIndex=1,nTrajectories=10)
  
  ppc=11-nExcluded #ppc gives the number of data points use per country.
  
  #Initialize result matrix.
  result=matrix(0,nrow=nTrajectories,ncol=nExcluded+18);
  ...
  nParams=paramEstimates[1];
  nSimPerChain=paramEstimates[2];
  paramMatrix=matrix(0,nrow=nParams,ncol=nTrajectories);
  paramMatrix[,]=paramEstimates[(3+500*nParams):(2+(nTrajectories+500)*nParams)]
  ...
  z$rates=matrix(0,nrow=nTrajectories,ncol=n);
  ...
  #Recover and store trajectory points from the prediction.
  result[,1]=newRates[,countryIndex]
  cat("Prediction 1 Complete.\n");
  ...
  for(t in 2:(18+nExcluded)){#18 points if 1 excluded, 20 points if 3 excluded
    ...
    #Recover and store trajectory points from the prediction.
    result[,t]=newRates[,countryIndex]
