...
paramFile="./PE/PE_Hier111_3Exc";outFile="./CI/CI_Hier111_3Exc.txt";nExcluded=3;truncation=c(-80,150);
nChains=4;CIWidths=c(0.8,0.95);
...
CImain=function(paramFile,nChains,nExcluded,CIWidths,outFile,truncation){
  setup();
  CIArrays=getCIs(paramFile,nChains,nExcluded,CIWidths,outFile,truncation);
}

getCIs=function(paramFile,nChains,nExcluded,CIWidths=0.8,outFile,truncation){
  ...
  nu=t(paramMatrix[(n+1):(2*n),]);
  phi=t(paramMatrix[(2*n+1):(3*n),]);
  tau=t(paramMatrix[(3*n+1):(4*n),]);
  sigma=tau^(-1/2);
...
evaluateCIFromFile=function(fileName,nExcluded){
  #Call evaluateCIFromFile("Hier111_3Exc",3)
  ciArray=getCIFromFile(fileName,nExcluded);
  evaluate(ciArray,nExcluded);  
}

CImain(paramFile,nChains,nExcluded,CIWidths,outFile,truncation)